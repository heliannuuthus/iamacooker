...
from app.types.models import DayPlan, GroceryItem, MealPlan, Recipe
...
def register_recommend_meals_tool(server: McpServer, recipes: List[Recipe]):
    ...
    @server.tool(
        ...
        params={
            "allergies": z.array(z.string(description="过敏原，例如'大蒜'"))
            .optional()
            .describe("过敏原列表，如['大蒜', '虾']"),
            "avoid_items": z.array(z.string(description="忌口食材，例如'葱'"))
            .optional()
            .describe("忌口食材列表，如['葱', '姜']"),
            "people_count": z.number(description="用餐人数，1-10之间的整数")
            .int()
            .min(1)
            .max(10),
        },
    ...
    async def recommend_meals(
        people_count: int, allergies: List[str] = None, avoid_items: List[str] = None
    ):
        ...
        for i in range(2):
            day_plan = DayPlan(
                day=["周六", "周日"][i], breakfast=[], lunch=[], dinner=[]
            )
            ...
            def get_meals(count: int):
                ...
                for _ in range(count):
                    ...
                    for category in categories[:]:
                        if (
                            category in recipes_by_category
                            and recipes_by_category[category]
                        ):
                    ...
                    if not selected:
                        ...
                        other_categories = [
                            c for c in recipes_by_category if recipes_by_category[c]
                        ]
                        if other_categories:
        ...
        # 根据食材类型分类食材
        categorize_ingredients(
            meal_plan.grocery_list.ingredients, meal_plan.grocery_list.shopping_plan
        )
            ...
            "content": [
                {
                    "type": "text",
                    "text": json.dumps(
                        meal_plan.model_dump(), ensure_ascii=False, indent=2
                    ),
                },
            ],
        }
